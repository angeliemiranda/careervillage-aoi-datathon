    ...
    >
      ...
      >
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 sticky top-0">
          <div className="flex justify-between items-start">
            <div className="flex-1">
              ...
              <p className="text-blue-100 text-lg">{job.company || 'Company Not Listed'}</p>
            </div>
            <button
              onClick={onClose}
          ...
          {job.match_score && (
            <div className="mt-4 inline-block bg-white/20 backdrop-blur px-4 py-2 rounded-full">
              <span className="font-semibold">{job.match_score.toFixed(0)}% Match</span>
            </div>
          )}
        </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <p className="font-semibold text-gray-900">
                {job.city && job.state ? `${job.city}, ${job.state}` : job.location || 'Not specified'}
              </p>
            </div>
            <div>
              <p className="text-sm text-gray-500 mb-1">Employment Type</p>
              <p className="font-semibold text-gray-900">{job.employment_type || 'Not specified'}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 mb-1">Industry</p>
              <p className="font-semibold text-gray-900">{job.industry || 'Not specified'}</p>
            </div>
          </div>

          {job.remote_work && (
            <div className="mb-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-blue-800 font-medium">🏠 Remote work available</p>
            </div>
          )}
          ...
          {job.aoi_score && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Opportunity Scores</h3>
              <p className="text-sm text-gray-600 mb-3">Based on American Opportunity Index data</p>
              <div className="space-y-3">
                {[
                  { label: 'Overall Score', value: job.aoi_score, description: 'Combined opportunity rating' },
                  { label: 'Wage Score', value: job.aoi_wage_score, description: 'Compensation potential' },
                  { label: 'Mobility Score', value: job.aoi_mobility_score, description: 'Career growth opportunity' },
                  { label: 'Access Score', value: job.aoi_access_score, description: 'Ease of entry' },
                  { label: 'Job Quality', value: job.aoi_job_quality_score, description: 'Overall job quality' },
                ].map(
                  (score) =>
                    score.value && (
                      <div key={score.label}>
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className="text-sm font-bold text-blue-600">{score.value.toFixed(1)}/5.0</span>
                        </div>
